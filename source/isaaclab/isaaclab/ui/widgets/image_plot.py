...
class ImagePlot(UIWidgetWrapper):
    ...
    它有以下布局，其中模式框架仅适用于深度图像:
    +-------------------------------------------------------+ | containing_frame |
    |+-----------------------------------------------------+| | main_plot_frame |
    ||+---------------------------------------------------+|| |||
    plot_frames信息信息信息信息信息信息信息信息信息信息信息信息mode_frame产品: 产品
    选择] 时时时时时时时时时时时时时时时
    """
    ...
    def __init__(
        ...
        """
        """创建一个XY图片 UI Widget，以轴扩展，传奇和支持多个图片。

        在框架中集中时，重叠的数据最准确地绘制。
        按下鼠标给出每个函数的x和y值在x坐标上。

        参数：
            image: 图像显示
            label: 图片左边的简短描述文本
            widget_height: 在像素中图片的高度
            min_value: 手动正常化/配色的最低值。
                       默认为0.0。
            max_value: 手动正常化/配色的最大值。
                       默认到1.0。
        """
    ...
    def _build_mode_frame(self):
        ...
        这是一个内部函数来构建包含地图模式选择的框架。
        这种函数只应从框架的构建函数内部调用。

        构建的小工具有以下布局: +-------------------------------------------------------+ \ mode_frame \ \ ˇ \ \ \ \ \
        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \
        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \
        \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \
        规范化选项] 手动的使用方式
