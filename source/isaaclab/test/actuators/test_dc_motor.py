...
def test_dc_motor_clip(num_envs, num_joints, device, test_point):
    ...
    ============================================================
    """
    """测试直流动机执行器4矩阵扭矩速度曲线的计算。
    torque_speed_pairs有关:

    0 - 完全内扭矩时速曲线和力度限制 (四分之一) 1 - 超过力度限制但在扭矩时速曲线下 (四分之一) 2 - 超过力度限制和外部扭矩时速曲线 (四分之一) 3 - 低于力度限制但在外扭矩时速曲线
    (四分之一) 4 - 低于力度限制但在外扭矩时速曲线和外角速度(四分之一) 5 - 完全内扭矩时速曲线和力度限制 (四分之一) 6 - 完全在外扭矩时速曲线和力度限制 (四分之一) 7 -
    完全在内扭矩时速曲线，在外扭矩时速限制，在外扭矩时速限制和在外角值 (四分之一) 8 - 完全在内扭矩时速曲线，在外扭矩时速限制，在外扭矩时速限制和在外角值 (四分之一) 9 -
    完全在内扭矩时速限制但在内扭矩时速曲线和外角值 (四分之一)effort_limit的saturation_effort其他velocity_limitc - 角速度 \ -
    每个v和s之间的扭矩速度线性界限torque_speed_point将在四方位进行测试.--e-----\ \ | \ \ | 0 \ 3 \ | \ (-)-----------v
    -------------o-------------v速度 速度 速度 速度--e---------------------c让我们一起来看看。
    """
    effort_lim = 60
    saturation_effort = 100.0
    velocity_limit = 50
